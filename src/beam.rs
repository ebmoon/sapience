...
impl PartialLibCost {
    pub fn new(beam_size: usize, lps: usize) -> PartialLibCost {
        PartialLibCost {
            beam_size,
            lps,
        }
    ...
    fn make(egraph: &EGraph<AstNode<Op>, Self>, enode: &AstNode<Op>) -> Self::Data {
        ...
        match Teachable::as_binding_expr(enode) {
            Some(BindingExpr::Lib(id, f, b)) => {
                ...
                let mut e = x(b).add_lib(id, x(f), analysis.lps);
                e.unify();
                e.prune(analysis.beam_size, analysis.lps);
                e
            }
            Some(_) | None => {
